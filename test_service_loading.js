...
// Test function to check service availability
function testServiceLoading() {
  console.log('=== Testing Service Loading ===');
  
  // Check if services are available with correct names
  const services = {
  ...
  // Test the checkServices function
  ...
  if (window.PlanWiseAPI) {
    ...
    console.log('PlanWiseAPI methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(window.PlanWiseAPI)));
  } else {
  ...
  if (window.PlanWiseScheduler) {
    ...
    console.log('PlanWiseScheduler methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(window.PlanWiseScheduler)));
  } else {
  ...
  // Test debug helpers
  if (window.apiDebug) {
    console.log('✅ apiDebug is available');
  } else {
    console.log('❌ apiDebug is not available');
  }
  
  if (window.schedulerDebug) {
    console.log('✅ schedulerDebug is available');
  } else {
    console.log('❌ schedulerDebug is not available');
  }
  ...
  return {
    ...
    checkServicesResult,
    allServicesAvailable: checkServicesResult
  };
}
...
function testAPIFunctionality() {
  ...
  if (!window.PlanWiseAPI) {
    console.log('❌ PlanWiseAPI not available for testing');
    return false;
  }
...
function testSchedulerFunctionality() {
  ...
  if (!window.PlanWiseScheduler) {
    console.log('❌ PlanWiseScheduler not available for testing');
    return false;
  }
...
async function runServiceTests() {
  ...
  return {
    serviceLoading: serviceLoadingTest,
    initialization: initializationTest,
    apiFunctionality: apiTest,
    schedulerFunctionality: schedulerTest,
